...
```{r convert state ref ID to two digit codes}
...
aqas_aqim_func <- function(data, pathway_name, state_column_name, city_column_name) {
  data |> dplyr::select(AQIM_FORM_ID,
                all_of(state_column_name),
                all_of(city_column_name),
                STATE_CD
                ) |> 
    collect() |> 
...
```{r separate AQAS land border data to NB-SB aqas}
aqas_aqim <- aqas_aqim %>%
  mutate(
    PATHWAY = case_when(
...
aqim_data <- rbind(aqas_aqim |> dplyr::select(-STATE_CD),
                   arm_aqim |> dplyr::select(-INSPECTION_LOCATION_STATE_CODE)
                   )
```
...
bandwidth = 2.5*params$US_grid_cell_size #in meters; for SpatialKDE::create_grid_hexagonal here and for use in KDE function in later chunk

#create a raster of US
us_bounds <- tigris::states(cb = FALSE) |> 
...
us_grid <- SpatialKDE::create_grid_hexagonal(
  ...
  only_inside = FALSE,
  side_offset = bandwidth
)
us_grid <- sf::st_intersection(us_bounds_sf, us_grid)

saveRDS(us_bounds_sf, here::here("output","intermediate files", "US hex grid","us_bounds.rds"))
saveRDS(us_grid, here::here("output","intermediate files", "US hex grid","us_grid.rds"))

```

```{r limit destinations to within US}
# Use sparse matrix approach for memory efficiency
inside <- lengths(st_within(aqim_data, us_grid)) > 0
...
# First, let's get the pathway names in the same order as they appear in the groups
# Get the pathway name for each group from the first row of each group
pathway_names <- pathway_groups %>% 
  purrr::map_chr(~.x$PATHWAY[1]) %>%
  unique()

# Verify that the order matches
for (i in seq_along(pathway_groups)) {
  cat("Group", i, "is", pathway_names[i], "\n")
}


# bandwidth created create US grid chunk
...
```{r show the spatial distribution for example pathway}
#Check the validity of the scaled KDE layers

#Where are most of the people going?
check <- aqim_data %>% filter(PATHWAY == "Pedestrian SB") %>% count(destination) %>% arrange(desc(n))
View(head(check, 10))
print(head(check, 10))
#mostly going to AZ, TX, CA

#And where does the map show people going?
setwd(here::here("output","proportional destination layers"))
...
test_layer <- SpatialKDE::kde(
    points = pathway_groups[[16]], #Pedestrian SB
    band_width = bandwidth,
    scaled = FALSE,
    grid = us_grid
  )
mapview::mapview(test_layer)

scaled_test_layer <- test_layer
scaled_test_layer$kde_value <- proportions(test_layer$kde_value)
mapview::mapview(scaled_test_layer)


#This is working fine
# but it is not reflecting the data that is saved in the scaled KDE files via the for loop above
```
