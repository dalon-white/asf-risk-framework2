...
# Now bind the dataframes with consistent types
...
# Apply the function to each pathway in arm_aqim_records
system.time(aqas_aqim <- aqas_aqim %>%
  rowwise() %>%
  mutate(entry_type = get_entry_type(PATHWAY)) %>%
  ungroup()
)
...
#print(aqim_data |> count(entry_type, PATHWAY))
...
# save data
```{r save aqim data}
write.csv(aqim_data, "C:/Users/Dalon.White/OneDrive - USDA/Desktop/Projects/asf-risk-framework-large-files/aqim pathway data prep.csv", row.names = FALSE)
...
```{r apply entry type to each data set}
# Apply the function to each pathway in arm_aqim_records
system.time(aqas_aqim <- aqas_aqim %>%
  rowwise() %>%
  mutate(entry_type = get_entry_type(PATHWAY)) %>%
...
dat_arm <- size_ref_func(data = dat_arm, reference_table = size_ref)
```
# save arm and aqas data separtely
```{r save aqas aqim data}
write.csv(dat, "aqas_aqim_combined.csv", row.names = FALSE)

```
```{r save arm aqim data}
write.csv(dat_arm, "arm_aqim_combined.csv", row.names = FALSE)  
```
