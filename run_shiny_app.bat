...
"C:\Program Files\R\R-4.2.1\bin\R.exe" -e "shiny::runApp('ASF risk map shiny app.R', launch.browser = TRUE)"
if %ERRORLEVEL% NEQ 0 (
    echo Failed to run Shiny app with default R path
    echo Trying alternative paths...
    
    if exist "C:\Program Files\R\R-4.1.0\bin\R.exe" (
        "C:\Program Files\R\R-4.1.0\bin\R.exe" -e "shiny::runApp('ASF risk map shiny app.R', launch.browser = TRUE)"
    ) else if exist "C:\Program Files\R\R-4.0.0\bin\R.exe" (
        "C:\Program Files\R\R-4.0.0\bin\R.exe" -e "shiny::runApp('ASF risk map shiny app.R', launch.browser = TRUE)"
    ) else (
